...
A talented RB2 may earn complementary work, but true-committee status requires independent hierarchy parity.
"""
...
def main():
    ...
    for team,g in m[elig].groupby('team'):
        ...
        # A true committee requires genuine role parity, not merely a talented RB2. As RB1 protection rises, the challenger must be progressively closer to the lead in hierarchy evidence.
        parity_ratio_req=.90+.05*cp
        parity_gap_req=.075-.035*cp
        parity=(hs>=.60 and hratio>=parity_ratio_req and hgap<=parity_gap_req)
        true=parity and evidence>=.60 and security<.65 and challenge>=hurdle and effective_evidence>=.40
        oneb=(not true) and ((hs>=.34 and evidence>=.44) or (depths[second]<=2 and hs>=.30 and mr>=.88)) and security<.86; tier='true_committee' if true else ('lead_plus_1b' if oneb else 'clear_lead'); protected=cp>=.50
    ...
    watch=['Bhayshul Tuten','TreVeyon Henderson','Rico Dowdle','Jeremiyah Love','Jadarian Price','Kenneth Walker III','Jahmyr Gibbs']; cols=['name','team','projected_rush_attempts','projected_targets','ppr_points','ppr_pos_rank','rb_committee_tier','rb_committee_rb2_hierarchy_score','rb_committee_rb2_talent_score','rb_committee_hierarchy_ratio','rb_committee_hierarchy_gap','rb_committee_hierarchy_evidence','rb_committee_carry_delta','rb_committee_target_delta']; print('RB_COMMITTEE_AUDIT',m[m.name.isin(watch)][cols].to_dict('records'))
