...
and reduces redistribution magnitude, avoiding threshold cliffs. Committee losses are bounded by the
upstream hierarchy evidence so later logic cannot erase a strong role projection.
"""
...
def main():
    ...
    for team,g in m[elig].groupby('team'):
        ...
        protection=np.clip(.28*(depths[lead]==1)+.20*starters[lead]+.20*confs[lead]+.16*np.clip(rcs[lead]/.50,0,1)+.10*np.clip(rts[lead]/.12,0,1)+.06*floors[lead],0,1); cp=float(np.clip((protection-.42)/.48,0,1)); effective_evidence=float(np.clip(evidence*(1-.46*cp),0,1)); challenge=float(np.clip(.38*sc+.24*ratio+.22*mr+.16*evidence,0,1)); hurdle=.61+.18*cp
        true=((sc>=.48 and evidence>=.60 and security<.68) or (sc>=.43 and ratio>=.78 and mr>=.92 and evidence>=.58 and security<.66)) and challenge>=hurdle and effective_evidence>=.43; oneb=(not true) and ((sc>=.36 and evidence>=.48) or (depths[second]<=2 and sc>=.32 and mr>=.88 and evidence>=.46)) and security<.84; tier='true_committee' if true else ('lead_plus_1b' if oneb else 'clear_lead'); protected=cp>=.50
        ...
        if tier=='lead_plus_1b':bc=np.clip((.07+.08*evidence)*damp,.025,.14); bt=np.clip((.08+.09*evidence)*damp,.03,.16); maxcl=min((.08*(1-.65*cp)+.02),max_loss_rate)*oldc[lead]; maxtl=min((.10*(1-.65*cp)+.025),max_target_loss)*oldt[lead]
    ...
    watch=['Bhayshul Tuten','TreVeyon Henderson','Rico Dowdle','Jeremiyah Love','Jadarian Price','Kenneth Walker III','Jahmyr Gibbs']; cols=['name','team','projected_rush_attempts','projected_targets','ppr_points','ppr_pos_rank','rb_committee_tier','rb_committee_hierarchy_evidence','rb_committee_max_carry_loss_rate','rb_committee_max_target_loss_rate','rb_committee_carry_delta','rb_committee_target_delta']; print('RB_COMMITTEE_AUDIT',m[m.name.isin(watch)][cols].to_dict('records'))
