...
Projection-only layer. Uses nflverse weekly player stats to compare full-season target
usage with final-8, final-6 and final-4 regular-season windows. Players showing a
sustained late-season role expansion receive more weight on recent usage; stable
players remain anchored to the full season. Team target totals are preserved.
"""
...
def main():
    ...
    if 'week' not in w.columns:raise RuntimeError('Weekly stats missing week')
    w=w[w.position.isin(['WR','TE'])].copy();w['targets']=pd.to_numeric(w.targets,errors='coerce').fillna(0);w['target_share']=pd.to_numeric(w.get('target_share'),errors='coerce')
    w['name_key']=w['player_display_name'].fillna(w.get('player_name','')).map(norm);teamcol='recent_team' if 'recent_team' in w.columns else 'team';w['team_key']=w[teamcol].replace(TEAM_ALIAS)
    maxw=int(pd.to_numeric(w.week,errors='coerce').max());windows={'l8':range(max(1,maxw-7),maxw+1),'l6':range(max(1,maxw-5),maxw+1),'l4':range(max(1,maxw-3),maxw+1)}
    feat=None
    ...
    rr=r[cols].copy();rr['name_key']=rr.name.map(norm);rr['team_key']=rr.team_2026.replace(TEAM_ALIAS)
    rr=rr.merge(feat,on=['name_key','team_key'],how='left')
    ...
    m['recent_role_breakout']=False;m['recent_role_multiplier']=1.0;m['recent_role_score']=np.nan
    for team,g in m[eligible].groupby('team'):
        ...
        season_tpg=pd.to_numeric(m.loc[idx,'targets_per_game'],errors='coerce').fillna(0);season_share=pd.to_numeric(m.loc[idx,'target_share'],errors='coerce').fillna(0)
        l8t=pd.to_numeric(m.loc[idx,'tpg_l8'],errors='coerce').fillna(season_tpg);l6t=pd.to_numeric(m.loc[idx,'tpg_l6'],errors='coerce').fillna(l8t);l4t=pd.to_numeric(m.loc[idx,'tpg_l4'],errors='coerce').fillna(l6t)
        l8s=pd.to_numeric(m.loc[idx,'share_l8'],errors='coerce').fillna(season_share);l6s=pd.to_numeric(m.loc[idx,'share_l6'],errors='coerce').fillna(l8s);l4s=pd.to_numeric(m.loc[idx,'share_l4'],errors='coerce').fillna(l6s)
        starter=m.loc[idx,'depth_starter'].fillna(False).astype(bool);rank=pd.to_numeric(m.loc[idx,'depth_rank'],errors='coerce').fillna(9)
        tpg_recent=.30*l8t+.40*l6t+.30*l4t;share_recent=.30*l8s+.40*l6s+.30*l4s
        ...
        # Stable players use mostly full-season evidence. Breakouts use recent role heavily.
        recent_weight=np.where(breakout,.72,.28)
        season_score=.55*season_tpg+.45*(season_share*30.0)
        recent_score=.55*tpg_recent+.45*(share_recent*30.0)
        score=(1-recent_weight)*season_score+recent_weight*recent_score
        score=pd.Series(score,index=idx).clip(lower=.25)
        current_share=cur/cur.sum();desired=score/score.sum()
        exposed=np.where(breakout,.42,.18)
        blended=current_share*(1-exposed)+desired*exposed
        mult=(blended/current_share.replace(0,np.nan)).replace([np.inf,-np.inf],np.nan).fillna(1)
        # Allow legitimate breakouts more room upward, but preserve a floor for teammates.
        lo=np.where(breakout,.88,.86);hi=np.where(breakout,1.32,1.16);mult=np.minimum(np.maximum(mult,lo),hi)
        new=cur*mult;new*=cur.sum()/new.sum()
        for j,i in enumerate(idx):
            old=num(m.at[i,'projected_targets'],0);nt=float(new.loc[i])
            if old>0:
                ratio=nt/old;scale(m,i,ratio);m.at[i,'recent_role_multiplier']=ratio;m.at[i,'recent_role_breakout']=bool(breakout.iloc[j]);m.at[i,'recent_role_score']=float(sustained.iloc[j])
    ...
    drop=['team_2026','targets_per_game','target_share','depth_starter','depth_rank','tpg_l8','share_l8','tpg_l6','share_l6','tpg_l4','share_l4'];m=m.drop(columns=[c for c in drop if c in m.columns],errors='ignore');nums=m.select_dtypes(include=[np.number]).columns;m[nums]=m[nums].round(3);m.to_csv(a.out,index=False)
    watch=m[m.name.isin(['Parker Washington','Jakobi Meyers','Brian Thomas Jr.'])];print(watch[['name','projected_targets','recent_role_breakout','recent_role_multiplier','recent_role_score','ppr_points','ppr_pos_rank']].sort_values('ppr_pos_rank').to_dict('records'));print('Recent-role breakouts:',int(m.recent_role_breakout.sum()));print(f'Wrote recent-role projections to {a.out}')
