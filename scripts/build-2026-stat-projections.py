...
This is the first projection stage that produces football counting stats and all
three fantasy scoring formats. It consumes the auditable 2026 team/role context
and 2025 player efficiency features.

Important: rookies and players without matched 2025 history remain explicitly
unprojected here. They require the dedicated rookie/manual-role pathway rather
than guessed NFL efficiency or opportunity.
"""
...
def regress(value, mean, amount):
    if pd.isna(value):
        return mean
    return (1 - amount) * float(value) + amount * float(mean)


def scoring(row, reception_points):
    return (
        0.04 * row.get("projected_passing_yards", 0)
        + 4 * row.get("projected_passing_tds", 0)
        - 2 * row.get("projected_interceptions", 0)
        + 0.10 * row.get("projected_rushing_yards", 0)
        + 6 * row.get("projected_rushing_tds", 0)
        + reception_points * row.get("projected_receptions", 0)
        + 0.10 * row.get("projected_receiving_yards", 0)
        + 6 * row.get("projected_receiving_tds", 0)
    )
...
def main():
    ...
    latest["name_key"] = latest["player_display_name"].fillna(latest.get("player_name", "")).str.lower().str.replace(r"[^a-z0-9]", "", regex=True)

    # Use league-wide 2025 efficiency as regression anchors by position.
    means = {}
    ...
    # Role context already carries most 2025 efficiency fields through, but merge
    # the remaining raw efficiency columns here by normalized name/position.
    ...
    # Attach the history-only PPR prior for diagnostics, not as a direct scoring input.
    if "player_display_name" in hist.columns:
        hist = hist.copy()
        hist["name_key"] = hist["player_display_name"].fillna(hist.get("player_name", "")).str.lower().str.replace(r"[^a-z0-9]", "", regex=True)
    ...
    for _, r in base.iterrows():
        ...
        row = {
            ...
            "projection_status": "manual_role_required" if manual else "modeled_veteran",
        }

        # Initialize common stat fields.
        for c in [
            ...
            row[c] = np.nan if manual else 0.0
        ...
        if pos == "QB":
            team_att = float(d.get("projected_pass_attempts") or 0)
            share = d.get("role_qb_attempt_share_2026")
            share = 0.97 if pd.isna(share) else float(np.clip(share, 0.20, 1.0))
            attempts = team_att * share
            ...
            rush_pg = 3.5 if pd.isna(rush_pg) else max(0, float(rush_pg))
            ...
            rush_total = float(d.get("projected_rush_attempts") or 0)
            pass_total = float(d.get("projected_pass_attempts") or 0)
            carry_share = d.get("role_carry_share_2026")
            target_share = d.get("role_target_share_2026")
            carry_share = 0.35 if pd.isna(carry_share) else float(np.clip(carry_share, 0.01, 0.90))
            target_share = 0.08 if pd.isna(target_share) else float(np.clip(target_share, 0.005, 0.35))
            carries = rush_total * carry_share
            targets = pass_total * target_share
            ...
            row.update({
                "projected_rush_attempts": carries,
                "projected_rushing_yards": carries * ypc,
                "projected_rushing_tds": carries * rush_td_rate,
                "projected_targets": targets,
                "projected_receptions": targets * catch,
                "projected_receiving_yards": targets * ypt,
            ...
            target_share = default_share if pd.isna(target_share) else float(np.clip(target_share, 0.005, 0.38))
            targets = pass_total * target_share
    ...
    modeled = result[result["projection_status"].eq("modeled_veteran")].copy()
    for fmt in ["ppr_points", "half_ppr_points", "standard_points"]:
        rank_col = fmt.replace("_points", "_overall_rank")
        result[rank_col] = result[fmt].rank(method="min", ascending=False)
        for pos in ["QB", "RB", "WR", "TE"]:
            mask = result["position"].eq(pos) & result[fmt].notna()
            pos_col = fmt.replace("_points", "_pos_rank")
            result.loc[mask, pos_col] = result.loc[mask, fmt].rank(method="min", ascending=False)

    # Round output only after all calculations/ranks are complete.
    ...
    print(f"Manual/rookie rows: {int((result['projection_status'] != 'modeled_veteran').sum())}")
    for pos in ["QB", "RB", "WR", "TE"]:
