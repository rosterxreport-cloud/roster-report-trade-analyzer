#!/usr/bin/env python3
"""Archetype-aware catch regression plus role-based receiving TD allocation."""
...
def main():
 ...
 # Catch-rate archetype curve from 2023-25 WR seasons. QB context is already in stats.
 wr=f[f.position.eq('WR')].copy();wr['catch_rate']=pd.to_numeric(wr.get('catch_rate'),errors='coerce');wr['air_yards_per_target']=pd.to_numeric(wr.get('air_yards_per_target'),errors='coerce');wr['targets']=pd.to_numeric(wr.get('targets'),errors='coerce');sample=wr[wr.targets.ge(30)&wr.catch_rate.notna()&wr.air_yards_per_target.notna()];coef=np.polyfit(sample.air_yards_per_target.clip(3,20),sample.catch_rate,2)
 ...
 for i,row in s[eligible&s.position.eq('WR')].iterrows():
  h=wr[wr.name_key.eq(norm(row['name']))].copy()
  if h.empty:continue
  h['w']=h.season.map(SW).fillna(0)*np.sqrt(h.targets.clip(lower=1)/80).clip(upper=1);ok=h.catch_rate.notna()&h.air_yards_per_target.notna()&h.w.gt(0)
  if not ok.any():continue
  hist=float(np.average(h.loc[ok,'catch_rate'],weights=h.loc[ok,'w']));adot=float(np.average(h.loc[ok,'air_yards_per_target'],weights=h.loc[ok,'w']));arch=float(np.clip(np.polyval(coef,np.clip(adot,3,20)),.48,.78));qb=float(np.clip(arch+.018*num(row.get('qb_environment_delta'),0),.45,.82));cr=float(np.clip(.50*hist+.30*arch+.20*qb,.45,.82));s.at[i,'catch_rate_history']=hist;s.at[i,'catch_rate_archetype']=arch;s.at[i,'catch_rate_qb_component']=qb;s.at[i,'catch_rate_regressed']=cr;s.at[i,'catch_rate_regression_applied']=True;s.at[i,'projected_receptions']=num(row.get('projected_targets'))*cr
 ...
 for team,g in m[eligible&m.position.isin(['RB','WR','TE'])].groupby('team'):
  ...
  idx=g.index;tar=pd.to_numeric(m.loc[idx,'projected_targets'],errors='coerce').fillna(0);yds=pd.to_numeric(m.loc[idx,'projected_receiving_yards'],errors='coerce').fillna(0);starter=m.loc[idx,'depth_starter'].fillna(False).astype(bool);rank=pd.to_numeric(m.loc[idx,'depth_rank'],errors='coerce').fillna(9);hs=[hist.get(norm(m.at[i,'name']),{'tdr':LEAGUE,'adot':8.,'fd':.35}) for i in idx];tdr=pd.Series([h['tdr'] for h in hs],index=idx).clip(.012,.11);adot=pd.Series([h['adot'] for h in hs],index=idx).clip(3,20);fd=pd.Series([h['fd'] for h in hs],index=idx).clip(.15,.65);reg=.65*LEAGUE+.35*tdr
  tsh=tar/max(tar.sum(),1);ysh=yds/max(yds.sum(),1);air=((adot-8)/8).clip(-.5,1);first=((fd-.35)/.20).clip(-1,1);yprr=pd.to_numeric(m.loc[idx,'wr_yprr_2025'],errors='coerce').fillna(1.6) if 'wr_yprr_2025' in m else pd.Series(1.6,index=idx);expl=((yprr-1.6)/1).clip(-.6,1);raw=.40*tsh+.18*ysh+.14*(reg/reg.sum())+.10*((air+1)/(air+1).sum())+.10*((first+1.1)/(first+1.1).sum())+.08*((expl+1)/(expl+1).sum());role=np.where(starter,1.05,np.where(rank.le(2),.97,np.where(rank.le(3),.88,.74)));w=pd.Series(raw*role,index=idx).clip(lower=.001);share=w/w.sum();individual=(.58*(tar*reg)+.42*(yds/175))*role;alloc=teamtd*.98*share;new=.62*alloc+.38*individual;new*=np.clip(teamtd*.99/max(new.sum(),.01),.90,1.12);new=new.clip(0,14);m.loc[idx,'projected_receiving_tds']=new;m.loc[idx,'td_allocation_share']=share;m.loc[idx,'td_individual_prior']=individual;m.loc[idx,'td_role_score']=w;m.loc[idx,'td_air_yards_per_target']=adot;m.loc[idx,'td_first_down_rate']=fd
 ...
 m=m.drop(columns=[c for c in ['role_team','rec_td_per_target','target_share','depth_starter','depth_rank'] if c in m],errors='ignore');nums=m.select_dtypes(include=[np.number]).columns;m[nums]=m[nums].round(3);m.to_csv(a.out,index=False);watch=m[m.name.isin(['Khalil Shakir','Jakobi Meyers','Jameson Williams','Jaylen Waddle'])];print(watch[['name','projected_targets','catch_rate_regressed','projected_receptions','projected_receiving_tds','td_allocation_share','ppr_points','ppr_pos_rank']].to_dict('records'));print('catch-rate adjusted',int(m.catch_rate_regression_applied.sum()))
