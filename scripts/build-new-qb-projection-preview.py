...
for _,r in q.iterrows():
 ...
 rows.append({"rank":p["posRank"],"player":p["name"],"team":p["team"],"opponent":opponent,"passAttempts":round(patt,1),"completions":round(pcompn,1),"passYards":round(pyd,1),"passTD":round(pptd,2),"INT":round(pint,2),"yardsPerAttempt":round(pypa,2),"airYards":round(pair,1),"airYardsPerAttempt":round(paypa,2),"carries":round(pcar,1),"rushYards":round(pry,1),"rushTD":round(prtd,2),"weeklyPoints":round(fp,1),"weekly4PtPassTD":round(fp,1),"weekly6PtPassTD":round(fp+(pptd*2*mm*float(avail["week_factor"])),1),"ROSpointsPerGame":round(rosppg,1),"availabilityStatus":avail["status"],"weekAvailability":avail["week_factor"],"projectionSource":"2026 player stats + ranking role","starterStatus":("PROVISIONAL" if pt in PROVISIONAL_STARTER else "CONFIRMED"),"starterNote":PROVISIONAL_STARTER.get(pt)})
