#!/usr/bin/env python3
"""Reallocate receiving opportunity using demonstrated recent target hierarchy.

Generic projection-only layer. It rewards current starters who demonstrated recent
team target earning and gently discounts teammates whose current projection is being
carried by older/full-season history. Total team targets are preserved.
"""
...
def main():
    ...
    cols=[c for c in ['name','position','team_2026','targets_per_game','target_share','depth_starter','depth_rank','changed_team'] if c in r.columns]
    ...
    m['target_hierarchy_multiplier']=1.0
    for team,g in m[eligible].groupby('team'):
        ...
        pg=pd.to_numeric(m.loc[idx,'targets_per_game'],errors='coerce').fillna(0)
        share=pd.to_numeric(m.loc[idx,'target_share'],errors='coerce').fillna(0)
        starter=m.loc[idx,'depth_starter'].fillna(False).astype(bool)
        rank=pd.to_numeric(m.loc[idx,'depth_rank'],errors='coerce').fillna(9)
        # Recent demonstrated hierarchy score. Slot/outside labels are intentionally neutral;
        # current starter status plus actual target earning determine the adjustment.
        recent=(pg/pg.max() if pg.max()>0 else pg)*.60 + (share/share.max() if share.max()>0 else share)*.40
        role=np.where(starter,1.0,np.where(rank.le(2),.90,np.where(rank.le(3),.78,.62)))
        score=pd.Series(recent*role,index=idx).clip(lower=.05)
        current_share=cur/cur.sum(); desired=score/score.sum()
        # Only 24% of team targets are exposed to this layer; avoids overreacting to one year.
        blended=.76*current_share+.24*desired
        # Bound individual movement to +/-18% before preserving team total.
        mult=(blended/current_share.replace(0,np.nan)).replace([np.inf,-np.inf],np.nan).fillna(1).clip(.82,1.18)
        new=cur*mult; new*=cur.sum()/new.sum()
        for i in idx:
            old=num(m.at[i,'projected_targets'],0); nt=float(new.loc[i])
            if old>0:
                ratio=nt/old; scale(m,i,ratio);m.at[i,'target_hierarchy_multiplier']=ratio
    ...
    drop=['role_team','targets_per_game','target_share','depth_starter','depth_rank','changed_team'];m=m.drop(columns=[c for c in drop if c in m.columns],errors='ignore');nums=m.select_dtypes(include=[np.number]).columns;m[nums]=m[nums].round(3);m.to_csv(a.out,index=False)
    watch=m[m.name.isin(['Parker Washington','Jakobi Meyers','Brian Thomas Jr.','Ladd McConkey'])];print(watch[['name','projected_targets','target_hierarchy_multiplier','ppr_points','ppr_pos_rank']].sort_values('ppr_pos_rank').to_dict('records'));print(f'Wrote target-hierarchy projections to {a.out}')
